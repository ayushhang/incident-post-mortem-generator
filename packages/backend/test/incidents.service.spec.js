...
const incidents_service_1 = require("../src/incidents/incidents.service");
const prisma_service_1 = require("../src/common/prisma/prisma.service");
const common_1 = require("@nestjs/common");
const shared_1 = require("@incidents/shared");
describe("IncidentsService", () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get(incidents_service_1.IncidentsService);
    ...
    describe("create", () => {
        it("should create a new incident", async () => {
            const createInput = {
                ...
                severity: shared_1.IncidentSeverity.CRITICAL,
            ...
            const result = await service.create(createInput, mockUser.id);
    ...
    describe("findAll", () => {
        it("should return paginated incidents", async () => {
            ...
            const result = await service.findAll({ page: 1, limit: 20 }, shared_1.UserRole.EDITOR);
        ...
        it("should filter by severity", async () => {
            ...
            await service.findAll({ page: 1, limit: 20, severity: shared_1.IncidentSeverity.HIGH }, shared_1.UserRole.VIEWER);
            expect(prismaService.incident.findMany).toHaveBeenCalledWith(expect.objectContaining({
                where: expect.objectContaining({
                    severity: shared_1.IncidentSeverity.HIGH,
                }),
            }));
        });
    });
    ...
    describe("update", () => {
        it("should update incident if user is owner", async () => {
            ...
            const result = await service.update("incident-1", { title: "Updated Title" }, mockUser.id, shared_1.UserRole.EDITOR);
        ...
        it("should throw ForbiddenException if user is not owner and is VIEWER", async () => {
            ...
            jest
                .spyOn(prismaService.incident, "findUnique")
                .mockResolvedValueOnce(mockIncident);
            await expect(service.update("incident-1", { title: "Updated" }, mockUser.id, shared_1.UserRole.VIEWER)).rejects.toThrow(common_1.ForbiddenException);
        });
    });
    describe("delete", () => {
        it("should delete incident if user is owner", async () => {
            ...
            await service.delete("incident-1", mockUser.id, shared_1.UserRole.EDITOR);
        ...
        it("should throw NotFoundException if incident not found", async () => {
            jest.spyOn(prismaService.incident, "findUnique").mockResolvedValueOnce(null);
            await expect(service.delete("nonexistent", mockUser.id, shared_1.UserRole.EDITOR)).rejects.toThrow(common_1.NotFoundException);
        });
    });
});
//# sourceMappingURL=incidents.service.spec.js.map